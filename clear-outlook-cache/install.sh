#!/bin/bash
#
# install.sh — install (or update) the daily Outlook cache cleaner.
# Quits Outlook, clears its Caches + WebKit data, relaunches it. Daily at a
# time you choose (default 06:00). Idempotent — safe to run repeatedly.
#
...
LABEL="local.clearoutlookcache"
OLD_LABEL="com.krisarmstrong.clearoutlookcache" # legacy id — cleaned up on (re)install
SCRIPT="clear-outlook-cache.sh"
DEFAULT_HOUR=6
DEFAULT_MINUTE=0
# -----------------------------------------------------------------------------
...
if [ "$ALL_USERS" -eq 1 ]; then
  ...
  # Logging is per-user: the script writes to its own $HOME/Library/Logs at
  # runtime. The launchd domain to (re)load into is the console user's GUI
  # session, if someone is logged in right now.
  TARGET_UID="${SUDO_UID:-}"
else
  [ "$(id -u)" -ne 0 ] || {
...
# Strip a leading zero so "06" doesn't parse as octal, then validate range.
HOUR=$((10#$HOUR))
MINUTE=$((10#$MINUTE))
...
# In all-users mode the plist must NOT pin StandardOut/ErrorPath to one user's
# home — the agent runs as each user. The script self-logs to its own
# $HOME/Library/Logs at runtime, so we just let launchd discard stray output.
if [ "$ALL_USERS" -eq 1 ]; then
  LOG_KEYS=""
else
  ...
  LOG_KEYS="    <key>StandardOutPath</key>
    <string>$LOGS_DIR/clear-outlook-cache.out.log</string>
    <key>StandardErrorPath</key>
    <string>$LOGS_DIR/clear-outlook-cache.err.log</string>"
fi
...
# Clean up any legacy job, then (re)load the current one into the console
# user's GUI session if someone is logged in.
unload "$OLD_LABEL"
rm -f "$OLD_PLIST"
unload "$LABEL"
...
echo "  Watch log:  tail -f ~/Library/Logs/clear-outlook-cache.log"
if [ "$ALL_USERS" -eq 1 ]; then
