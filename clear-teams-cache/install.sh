...
# it if it was running. Daily at a time you choose (default 06:00). Idempotent.
#
...
LABEL="local.clearteamscache"
OLD_LABEL="com.krisarmstrong.clearteamscache" # legacy id — cleaned up on (re)install
...
echo "  Watch log:  tail -f ~/Library/Logs/clear-teams-cache.log"
if [ "$ALL_USERS" -eq 1 ]; then
